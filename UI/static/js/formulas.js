    ...
    formulasArray.forEach((formula, index) => {
        ...
        card.innerHTML = `
            <div class="card-header">
                <h5>🧮 ${formula.name} - ${formula.formula}</h5>
            </div>
            <div class="card-body">
                <p><strong>Что нужно найти:</strong></p>
                <div class="mb-3" id="target-select-${index}"></div>
                <p><strong>Введите известные значения:</strong></p>
                <div id="formula-inputs-${index}"></div>
                <button class="btn btn-primary mt-3" onclick="calculateFormula(${index}, '${formula.name}', '${formulasState.currentCategory}', '${formulasState.currentSubcategory}')">
                    🧮 ВЫЧИСЛИТЬ
                </button>
        ...
        // Создаем радио-кнопки для выбора целевой переменной
        const targetSelect = document.getElementById(`target-select-${index}`);
        formula.fields.forEach((field, fieldIndex) => {
            const [fieldId, fieldName, unit] = field;
            const div = document.createElement('div');
            div.className = 'form-check';
            div.innerHTML = `
                <input class="form-check-input" type="radio" name="target-${index}" id="target-${index}-${fieldId}" value="${fieldId}">
                <label class="form-check-label" for="target-${index}-${fieldId}">
                    ${fieldName}${unit ? ' [' + unit + ']' : ''}
                </label>
            `;
        ...
        formula.fields.forEach((field, fieldIndex) => {
            ...
            div.className = 'mb-2';
            div.innerHTML = `
                <label for="input-${index}-${fieldId}" class="form-label">${fieldName}${unit ? ' [' + unit + ']' : ''}</label>
                <input type="number" class="form-control" id="input-${index}-${fieldId}" 
                       step="0.0001" placeholder="Введите значение" data-field-id="${fieldId}">
            `;
...
function calculateFormula(index, formulaName, category, subcategory) {
    const targetRadio = document.querySelector(`input[name="target-${index}"]:checked`);
    if (!targetRadio) {
        alert('⚠️ Пожалуйста, выберите неизвестную величину');
        return;
    }
    ...
    .then(data => {
        ...
        if (data.error) {
            ...
            const fieldName = formula.fields.find(f => f[0] === target)[1];
            const unit = formula.fields.find(f => f[0] === target)[2];
            resultDiv.innerHTML = `
                <div class="alert alert-success">
                    ✅ <strong>Результат:</strong> ${fieldName} = ${data.result.toFixed(4)}${unit ? ' ' + unit : ''}
                </div>
            `;
        }
    })
